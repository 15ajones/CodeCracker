                ...
                if game_type == "mastermind":
                        ...
                        if not game_started:#if game has ended we can end the for loop and leave this section
                            ...
                            if correct_guess: # if correct guess we send a message saying game over to everyone, and end this section
                                ...
                                if UI_cadd != "none":
                                    ...
                                    cmsg = "leaderboard"
                                    for n in players:
                                        cmsg+=" " +str(query_user(str(player[1]))['Name'])+ " "+ str(int(query_user(str(n[1]))['info']['points']))
                                    server_socket.sendto(cmsg.encode(), UI_cadd)
                                    time.sleep(5)
                                brea
                    ...
                    if players_left == 1:
                        ...
                        for x in players:
                            server_socket.sendto(cmsg.encode(), (x[0], int(x[1])))
                        if UI_cadd != "none":
                            cmsg = "winner " +  str(query_user(str(winner[1]))['Name'])
                            server_socket.sendto(cmsg.encode(), UI_cadd)
                            time.sleep(4)
                            cmsg = "menu"
                            server_socket.sendto(cmsg.encode(), UI_cadd)
                    ...
                    for player in memory_players:
                        if UI_cadd != "none":
                            ...
                            for x in memory_players:
                                cmsg+=" "
                                cmsg+=str(query_user(str(x[1]))['Name'])
                            server_socket.sendto(cmsg.encode(), UI_cadd)
                            time.sleep(4)
                        if not game_started:#if game has ended we can end the for loop and leave this section
                            break
                        ...
                        if player == 0:
                            continue
                        if UI_cadd != "none":
                            cmsg = "turn " + str(query_user(player[1])['Name'])
                            server_socket.sendto(cmsg.encode(), UI_cadd)
                            time.sleep(4)
                        cmsg = "your turn"
                        server_socket.sendto(cmsg.encode(), (player[0], int(player[1])))
